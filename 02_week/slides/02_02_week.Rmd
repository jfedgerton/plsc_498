...
subtitle: "Reproducible workflow + four plots + short write-up"
...
- Produce **four** plots that illustrate different encodings
- Save plots to files
...
# 4D) Plot D: choose a different encoding (size encodes a quantitative variable)

```{r,fig.width=4, fig.height=2.8,  message=FALSE, warning=FALSE}
p4 <- ggplot(dat, aes(x = displ, y = hwy, size = cty)) +
...
ggsave(
  filename = "figures/plot_D_size_encodes_cty.png",
  plot = p4,
  width = 7, height = 4.5, dpi = 300
)
```
...
- `figures/plot_A_baseline_scatter.png`
- `figures/plot_B_color_by_class.png`
- `figures/plot_C_styling_constant_color.png`
- `figures/plot_D_size_encodes_cty.png`
...
- The four PNGs are saved in `figures/`
